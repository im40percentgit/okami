...
| DEC-OKAMI-021 | Multi-OS CI matrix (Ubuntu + macOS + Windows) with full `cargo test --all` green on all three; key-file protection (DEC-004 + DEC-018) is `cfg(unix)`-only and explicitly out-of-scope for Windows in 0.1.0; hybrid PQC keygen needs >1 MiB stack so `.cargo/config.toml` bumps Windows linker `/STACK` to 32 MiB, the CLI binary runs on a 32 MiB worker thread for portability, and CI sets `RUSTDOCFLAGS` on the Windows job for doctests | ACL-based protection equivalents on Windows are a separate design problem (out of scope for 0.1.0); documenting the explicit gap prevents silent regression. The stack-size workaround is necessary because Windows main threads default to 1 MiB while Linux/macOS default to 8 MiB, and rustdoc does not honor `target.cfg` rustflags so doctests need an env-var override. | `src/identity.rs`, `src/bin/okami.rs`, `.cargo/config.toml`, `.github/workflows/ci.yml` |
| DEC-OKAMI-022 | cargo-fuzz integration with libFuzzer targets for `DelegationChain::from_bytes`, `SpiffeId::parse`, `PqcCredential::from_bytes`, `SignedAuditEvent::from_bytes`; nightly CI cadence | proptest (DEC-012) gives partial coverage; libFuzzer's corpus-guided exploration is complementary. Contract is "no panics on arbitrary input"; allocation caps from DEC-016 must hold. Nightly is the right cadence — too slow for PR gating, cheap on cron. | `fuzz/`, `.github/workflows/fuzz.yml` |

## Review Status

CEO + ENG CLEARED. Phase 1 complete. 22 decisions documented. Security hardening pass (/cso 2026-04-24) merged across PRs #1-#6. Published as okami 0.2.0 on crates.io (2026-04-27).
